...
public class IntervalTree<Type> {
	...
	public List<Type> get(double time) {
		List<IntervalData<Type>> intervals = getIntervals(time);
		List<Type> result = new ArrayList<Type>();
		for(IntervalData<Type> interval : intervals)
	...
	public List<Type> get(long start, long end) {
		List<IntervalData<Type>> intervals = getIntervals(start, end);
		List<Type> result = new ArrayList<Type>();
		for(IntervalData<Type> interval : intervals)
			result.add(interval.getData());
		return result;
	 ...
	 */
	public List<IntervalData<Type>> getIntervals(long start, long end) {
		build();
		return head.query(new IntervalData<Type>(start, end, null));
	}
	...
	public void addInterval(long begin, long end, Type data) {
		intervalList.add(new IntervalData<Type>(begin, end, data));
		inSync = false;
	}
